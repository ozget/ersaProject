...
using Menu = ErsaProject.Domain.Entities.Identity.Menu;

namespace ErsaProject.Persistence.Services
{
    public class AuthorizationEndpointService : IAuthorizationEndpointService
    {
        readonly IApplicationService _applicationService; //Endpointleri getirmek icin
        ...
        // code karşılık endpoint varmı yokmu kontrol, yoksa veri tabanına ekleme
        ...
        {//ilk önce menü varmı
            ...
            if (endpoint == null)//code karşılık endpoint db kaydedilmemiştir
            {

                var datas = _applicationService.GetAuthorizeDefinitionEndpoints(type);

                var data = datas.FirstOrDefault(m => m.Name == menu);

                //code:GET.Reading.GetOrder
                //Authorize ile işaretlenmiş endpointleri elde etme
            ...
            //ilk önce var olan role siliyoruz
            foreach (var role in endpoint.Roles)
                endpoint.Roles.Remove(role);

            //rolleri datalarını buluyoruz
